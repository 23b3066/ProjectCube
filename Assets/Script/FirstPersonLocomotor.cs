...
namespace Oculus.Interaction.Locomotion
{
        ...
        ILocomotionEventHandler, IDeltaTimeConsumer
    {
        ...
        private void AddVelocity(Vector3 velocity)
        {
            ...
            float speedFactor = GetModifiedSpeedFactor();

            // 空中なら加速度を控えめにする or 加算しない
            if (!IsGrounded)
            {
                speedFactor *= 0.1f;  // または0fにしても良い
            }

            _velocity += velocity * speedFactor;
            // TryExitHotspot(true);
        ...
        {
            float baseFactor = _acceleration
